...
class FailingPlatformClient:
    ...
    def find_article_by_title(self, title: str) -> Tuple[Optional[str], Optional[bool]]:
        ...
        if self.failure_mode == "auth_error":
            raise AuthenticationError(f"Authentication failed for {self.platform_name}", platform=self.platform_name)
        elif self.failure_mode == "rate_limit":
            raise RateLimitError(f"Rate limit exceeded for {self.platform_name}", platform=self.platform_name)
        elif self.failure_mode == "api_error":
            raise APIError(f"API error for {self.platform_name}", platform=self.platform_name)
        elif self.failure_mode == "intermittent" and self.call_count <= 2:
            raise APIError(f"Intermittent error for {self.platform_name}", platform=self.platform_name)
    ...
    def publish_article(self, post_content, published: bool) -> Dict:
        ...
        if self.failure_mode == "auth_error":
            ...
            raise APIError(f"Timeout error for {self.platform_name}", platform=self.platform_name)
    ...
    def get_article(self, article_id: str, published: bool) -> Optional[Dict]:
        if self.failure_mode == "get_article_error":
            raise APIError(f"Get article error for {self.platform_name}", platform=self.platform_name)
        return None
    
    def get_articles(self) -> List[Dict]:
        if self.failure_mode == "get_articles_error":
            raise APIError(f"Get articles error for {self.platform_name}", platform=self.platform_name)
        return []
...
class TestErrorScenarios:
        ...
        class PartialFailureClient(FailingPlatformClient):
            ...
            def find_article_by_title(self, title: str):
                ...
                if self.find_calls == 1:
                    raise APIError(f"Find error for {self.platform_name}", platform=self.platform_name)
                return None, None
    ...
    def test_platform_validation_errors(self):
        ...
        auth_error_client = FailingPlatformClient("devto", "get_articles_error")  # Changed to get_articles_error
        get_articles_error_client = FailingPlatformClient("hashnode", "get_articles_error")
...
"""
    ...
    def test_malformed_frontmatter_handling(self):
        ...
        unusual_post = PostContent(
            ...
            tags=["tag with spaces", "tag,with,commas", "tag-with-dashes", "tag_with_underscores"],
...
"""
    ...
    def test_content_processor_with_empty_content(self):
        ...
        whitespace_content = "   \n\t  \n   "
        whitespace_normalized = processor.normalize_content_for_comparison(whitespace_content)
    ...
    def test_tag_conversion_edge_cases(self):
        ...
        processed_special = processor.convert_tags_for_platform(special_tags, "hashnode")
